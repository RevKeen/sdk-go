...
> CheckoutSessionRetrieveResponse CheckoutSessionsRetrieve(ctx, id).Execute()
...
func main() {
	...
	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	resp, r, err := apiClient.CheckoutSessionsAPI.CheckoutSessionsRetrieve(context.Background(), id).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `CheckoutSessionsAPI.CheckoutSessionsRetrieve``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `CheckoutSessionsRetrieve`: CheckoutSessionRetrieveResponse
	fmt.Fprintf(os.Stdout, "Response from `CheckoutSessionsAPI.CheckoutSessionsRetrieve`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiCheckoutSessionsRetrieveRequest struct via the builder pattern
