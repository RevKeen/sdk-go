...
[**PaymentIntentsRetrieve**](PaymentIntentsAPI.md#PaymentIntentsRetrieve) | **Get** /payment-intents/{id} | Retrieve a payment intent
...
> PaymentIntent PaymentIntentsRetrieve(ctx, id).Execute()
...
func main() {
	...
	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	resp, r, err := apiClient.PaymentIntentsAPI.PaymentIntentsRetrieve(context.Background(), id).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `PaymentIntentsAPI.PaymentIntentsRetrieve``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `PaymentIntentsRetrieve`: PaymentIntent
	fmt.Fprintf(os.Stdout, "Response from `PaymentIntentsAPI.PaymentIntentsRetrieve`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiPaymentIntentsRetrieveRequest struct via the builder pattern
