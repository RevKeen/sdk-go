...
type PaymentIntentsAPI interface {
	...
	*/
	...
	/*
	PaymentIntentsRetrieve Retrieve a payment intent

	Retrieves a payment intent by its ID (pi_xxx).

	@param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
	@param id Payment intent ID (pi_xxx)
	@return ApiPaymentIntentsRetrieveRequest
	*/
	PaymentIntentsRetrieve(ctx context.Context, id string) ApiPaymentIntentsRetrieveRequest

	// PaymentIntentsRetrieveExecute executes the request
	//  @return PaymentIntent
	PaymentIntentsRetrieveExecute(r ApiPaymentIntentsRetrieveRequest) (*PaymentIntent, *http.Response, error)
}
...
type ApiPaymentIntentsRetrieveRequest struct {
	ctx context.Context
	ApiService PaymentIntentsAPI
	id string
}

func (r ApiPaymentIntentsRetrieveRequest) Execute() (*PaymentIntent, *http.Response, error) {
	return r.ApiService.PaymentIntentsRetrieveExecute(r)
}
...
Retrieves a payment intent by its ID (pi_xxx).
 ...
 @return ApiPaymentIntentsRetrieveRequest
*/
func (a *PaymentIntentsAPIService) PaymentIntentsRetrieve(ctx context.Context, id string) ApiPaymentIntentsRetrieveRequest {
	return ApiPaymentIntentsRetrieveRequest{
		ApiService: a,
		ctx: ctx,
		id: id,
...
// Execute executes the request
//  @return PaymentIntent
func (a *PaymentIntentsAPIService) PaymentIntentsRetrieveExecute(r ApiPaymentIntentsRetrieveRequest) (*PaymentIntent, *http.Response, error) {
	var (
		localVarHTTPMethod   = http.MethodGet
		localVarPostBody     interface{}
		formFiles            []formFile
	...
	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "PaymentIntentsAPIService.PaymentIntentsRetrieve")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}
