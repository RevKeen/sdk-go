...
[**RefundsRetrieve**](RefundsAPI.md#RefundsRetrieve) | **Get** /refunds/{id} | Get refund by ID
...
> RefundRetrieveResponse RefundsRetrieve(ctx, id).Execute()
...
func main() {
	...
	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	resp, r, err := apiClient.RefundsAPI.RefundsRetrieve(context.Background(), id).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `RefundsAPI.RefundsRetrieve``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `RefundsRetrieve`: RefundRetrieveResponse
	fmt.Fprintf(os.Stdout, "Response from `RefundsAPI.RefundsRetrieve`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiRefundsRetrieveRequest struct via the builder pattern
