...
// UsageEvent A single usage event representing customer consumption of a metered resource. Provide one of customer_id, external_customer_id, or subscription_id. Idempotency is permanent per merchant via the usage_events merchant/idempotency unique index. Each subscription has one currency; usage events attributed to it are interpreted in that currency with no metering-layer FX conversion.
