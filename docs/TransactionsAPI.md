...
------------- | ------------- | -------------
[**TransactionsGet**](TransactionsAPI.md#TransactionsGet) | **Get** /transactions/{id} | Get transaction by ID
...
> TransactionRetrieveResponse TransactionsGet(ctx, id).Execute()
...
func main() {
	...
	resp, r, err := apiClient.TransactionsAPI.TransactionsGet(context.Background(), id).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `TransactionsAPI.TransactionsGet``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `TransactionsGet`: TransactionRetrieveResponse
